...
### LLM 을 언제 쓰는가 — 배열을 바꿔가며 재봤다

렌더링한 1페이지를 LLM 으로 읽으면 위첨자·기호·각주 배치를 한 번에 처리한다.
그러면 PDF 파서 여덟 개가 필요 없는 게 아닌가. ai4s 무작위 **300편**으로 두 배열을
비교하고(`pipeline/experiment_ladder_order.py`, 시드 고정), 채택한 배열은 코퍼스
**4,196편** 전체에 적용해 측정했다.
...
|---|---:|---:|
| 해결한 논문 | **240** | **240** |
| 저자↔기관 링크 | 1,051 | **1,186** (+13%) |
| 처리 시간 | **3.9초** | 30분 |
| 비용 | **$0** | $2.83 |
| 1,000편 환산 | **$0** | $9.44 |

**해결률이 완전히 같다.** 파서가 풀고 LLM 이 못 푼 논문도, 그 반대도 0편이었다.
둘 다 실패한 60편은 기관 행이나 저자 자체가 없어 어떤 판독기로도 안 된다.

차이는 **깊이**다. 두 배열이 갈린 103편에서 추가된 링크를 원문과 대조하면
**원문이 뒷받침한 것이 LLM 190건 vs 파서 87건**, 반대로 **원문에 없는 링크가
LLM 13건 vs 파서 1건**이다. 더 깊게 읽지만 잡음도 함께 늘어난다.

#### 2) 그래서 채택한 배열 — 필요한 논문만 다시 읽기

순서를 바꾸는 대신, 파서가 **저자의 80% 미만**만 매핑한 논문에 한해 페이지를
재판독한다(`--augment`). 대개 바이라인이 열 줄로 감기는 대형 협업에서 파서가 첫
줄만 읽은 경우다. 코퍼스 전체 적용 결과:

|  | LLM 을 최후에만 | **현재 — 최후 + 80% 미만 보강** |
|---|---:|---:|
...
전량 재판독(배열 2) 대비 비용은 **약 1/4**이고, 완결된 논문은 건드리지 않는다.

```bash
python pipeline/extract_byline_llm.py --augment --execute     # 일부만 잡힌 논문
python pipeline/experiment_ladder_order.py --sample 300       # 배열 재검증
```
