...
Zotero에서 논문을 가져와 Claude/Gemini API로 리뷰하고, Bottom-up 토픽 모델링(HDBSCAN + UMAP)으로 분류한 뒤, Cloudflare Workers(Static Assets)에 검색 가능한 HTML 인덱스로 배포합니다.
...
- **Landing**: [paper-curation.jehyun-lee.workers.dev](https://paper-curation.jehyun-lee.workers.dev/)
- **AI for Science**: [paper-curation.jehyun-lee.workers.dev/ai4s](https://paper-curation.jehyun-lee.workers.dev/ai4s/)
- **Science of Science**: [paper-curation.jehyun-lee.workers.dev/scisci](https://paper-curation.jehyun-lee.workers.dev/scisci/)

> 기존 `jehyunlee.github.io/paper-curation/*` 경로는 하위 path까지 보존하여 Cloudflare로 자동 리다이렉트됩니다.
...
8. **배포** — 네트워크 시각화 포함 검색 가능 HTML 인덱스 빌드, PNG→WebP 변환, Cloudflare Workers 자동 배포 (master push 시)
...
| 9 | `prepare_deploy.py` | PNG→WebP, .gitignore 업데이트, master push | Cloudflare 자동 배포 |
...
Papers are fetched from Zotero, reviewed via Claude/Gemini APIs, classified using bottom-up topic modeling (HDBSCAN + UMAP), and published as a searchable HTML index on Cloudflare Workers (Static Assets).
...
> The legacy `jehyunlee.github.io/paper-curation/*` URLs still work — every sub-path is automatically redirected to the Cloudflare deployment while preserving the path.
...
8. **Publish** — Build searchable HTML index with network visualization, PNG→WebP conversion, auto-deploy on Cloudflare Workers (triggered by `master` push)
