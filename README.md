...
### 왜 파서를 먼저 쓰고 LLM 을 나중에 쓰나

렌더링한 1페이지를 LLM 으로 읽으면 위첨자·기호·각주 배치를 한 번에 처리한다.
그러면 PDF 파서 여덟 개는 필요 없는 게 아닌가 — ai4s 무작위 **300편**으로 두
배열을 비교했다 (`pipeline/experiment_ladder_order.py`, 시드 고정).

|  | A: 파서 먼저 (현행) | B: LLM 먼저 |
|---|---:|---:|
| 해결한 논문 | **240** | **240** |
| 저자↔기관 링크 | 1,051 | **1,186** |
| 처리 시간 | **3.9초** | 30분 |
| 비용 | **$0** | **$2.83** (1,000편당 $9.44) |

**해결률은 완전히 같다.** 파서가 풀고 LLM 이 못 푼 논문도, 그 반대도 0편이었다.
둘 다 실패한 60편은 기관 행이나 저자 자체가 없어 어떤 판독기로도 안 된다.

차이는 **깊이**다. LLM 은 링크를 13% 더 만들고, 두 배열이 갈린 103편에서 추가된
링크를 원문과 대조하면 **원문이 뒷받침한 것이 LLM 190건 vs 파서 87건**이다.
다만 **원문에 없는 링크도 LLM 13건 vs 파서 1건** — 접지 관문을 통과했는데도
페이지에 없는 기관이 섞인다.

그래서 순서를 바꾸는 대신 **필요한 논문만 다시 읽는다.** 파서가 저자의 80% 미만만
매핑한 논문 — 대개 바이라인이 열 줄로 감기는 대형 협업에서 파서가 첫 줄만 읽은
경우 — 에 한해 `--augment` 로 페이지를 재판독한다. 전량 재판독 대비 비용은
**1/3 수준**이고, 완결된 논문은 건드리지 않는다.

```bash
python pipeline/extract_byline_llm.py --augment --execute
```

코퍼스 전체에 적용한 결과 — 대상 1,126편 중 1,120편을 읽어 링크 4,411개를 얻었다.
**저자 전원의 소속이 확정된 논문이 44.9% → 53.0%** 로 올랐고, 신뢰 링크는
24,449 → 26,212건, 기관이 확정된 저자는 13,270 → 15,280명이 됐다. 해결된 논문
수(86.5%)는 그대로다 — 보강은 새 논문을 푸는 게 아니라 **이미 푼 논문을 끝까지
읽는** 작업이기 때문이다.
