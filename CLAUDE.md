...
| 4.5 | `pipeline/topic_modeling.py` + `pipeline/extract_insights.py` + `pipeline/lib/related.py` | Core paper connections are deterministic: SPECTER2 cosine and title/author BM25 ranks are RRF-fused, then `build_connections` derives metadata-backed relation/reason without an LLM judge. Both Step 6 and the subsequent `extract_insights.py` connection pass use this path; the latter preserves category target scoping, whole-topic candidate pooling, and merged persistence. Cross-category Research Insights remains opt-in with `run_full --insights`; its backend fallback is Anthropic → OpenAI → Gemini. |
...
- **connections는 메타데이터 기반 휴리스틱이다**: RRF 후보 폭은 `EXTRACT_INSIGHTS_TOPN_CAND`로 조정하며, 후보는 LLM judge·connection cache·증분 재시도 없이 `build_connections`에 직접 전달된다. 출력 relation/reason은 공저자·발행 순서·카테고리·유사도에서 파생한 탐색 보조 정보이며, 확립된 인과 또는 인용 관계를 주장하지 않는다.
