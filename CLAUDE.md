...
Academic paper curation pipeline deployed as a GitHub Pages site. Papers are fetched from Zotero, reviewed via Claude/Gemini APIs, classified into categories, and published as a searchable HTML index with per-paper review pages.

- **Topics**: Configured per-user in `config.json` (e.g., `ai4s`, `scisci`, `bioml`)
- **Deployed to**: GitHub Pages from `master` branch (optional)
...
7. **GitHub Pages 설정** — 선택사항, 없으면 생략

### Step 3: setup.py 실행 및 검증
```bash
PYTHONUTF8=1 python pipeline/setup.py
```
setup.py가 6단계를 자동 수행한다:
- [1/6] config.json 로드
- [2/6] 환경변수 확인 (ANTHROPIC_API_KEY, GOOGLE_API_KEY)
...
| 8 | `pipeline/build_topic_index.py` | Generate `{topic}/index.html` with cards, search, timelines |
| 9 | `pipeline/cleanup.py` | Remove stale files (old timelines, graphify temp, caches) |
| 10 | `pipeline/prepare_deploy.py` | PNG→WebP, .gitignore update, optional git push |
...
- **Anthropic API**: Claude Haiku for classification, reviews, and summaries (`ANTHROPIC_API_KEY` env var)
